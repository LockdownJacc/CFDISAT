...
$config = include(app_path('Http\Controllers\php\config.php'));;
...
if(!empty($_POST)) {
  ...
  if(!empty($_POST['sesion'])) {
    $var->restaurarSesion($_POST['sesion']);
  }
  ...
  if($accion == 'login_ciec') {
    if(!empty($_POST['rfc']) && !empty($_POST['pwd'])) {
      ...
      $ok = $var->iniciarSesionCiec($_POST['rfc'],$_POST['pwd']);
      if($ok) {
        echo json_response(array(
          'mensaje' => 'Se ha iniciado la sesión',
          'sesion' => $var->obtenerSesion()
        ));
      }else{
        echo json_response(array(
    ...
    if(!empty($_POST['rfc']) && !empty($_POST['pwd']) && !empty($_POST['captcha'])) {
      ...
      // iniciar sesion en el SAT
      $ok = $var->iniciarSesionCiecCaptcha($_POST['rfc'],$_POST['pwd'],$_POST['captcha']);
      if($ok) {
        echo json_response(array(
          'mensaje' => 'Se ha iniciado la sesión',
          'sesion' => $var->obtenerSesion()
    ...
    if(!empty($_FILES['cer']) && !empty($_FILES['key']) && !empty($_POST['pwd'])) {
      ...
      if($ok) {
        // iniciar sesion en el SAT
        $ok = $var->iniciarSesionFiel($certificado);
        if($ok) {
          echo json_response(array(
            'mensaje' => 'Se ha iniciado la sesión',
            'sesion' => $var->obtenerSesion()
